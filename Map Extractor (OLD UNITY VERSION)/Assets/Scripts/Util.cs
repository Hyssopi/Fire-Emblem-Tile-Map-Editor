...
public class Util
{
  ...
  public static Texture2D Get15BitTexture(Texture2D texture)
  {
    ...
    {
      colors[i].r &= 0b1111_1000;
      colors[i].g &= 0b1111_1000;
      colors[i].b &= 0b1111_1000;
    }
  ...
  /// <returns></returns>
  /// <remarks>
  /// https://answers.unity.com/questions/1641752/texture2dloadimage-colorcolour-problems.html
  /// https://issuetracker.unity3d.com/issues/loading-a-png-file-with-resources-dot-load-and-texture2d-dot-loadimage-produces-different-results
  /// </remarks>
  public static Texture2D Get24BitTexture(Texture2D texture)
  {
    Texture2D texture24Bit = DuplicateTexture(texture);

    Color32[] colors = texture24Bit.GetPixels32();
    ...
    for (int i = 0; i < colors.Length; i++)
    {
      ...
      int redMostSignificantBits = (colors[i].r >> 5) & 0b0111;
      int greenMostSignificantBits = (colors[i].g >> 5) & 0b0111;
      int blueMostSignificantBits = (colors[i].b >> 5) & 0b0111;

      colors[i].r |= (byte)redMostSignificantBits;
      colors[i].g |= (byte)greenMostSignificantBits;
      colors[i].b |= (byte)blueMostSignificantBits;
    }

    texture24Bit.SetPixels32(colors);
    texture24Bit.Apply();
